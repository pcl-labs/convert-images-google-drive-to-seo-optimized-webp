...
1. **Structured outlines + chapters**
   - Replace the naive outline with a real heading/summary hierarchy (intro, body, CTA slots, keywords).
...
2. **Automatic pipeline execution**
   - Submitting a YouTube URL should enqueue ingest → outline → chapter shaping → AI compose → Drive sync automatically.
   - Emit pipeline events/SSE so users watch a todo list flip green without touching manual steps.
3. **Final handoff & UX**
   - When the AI draft lands, send a “Blog ready” notification with the Google Doc link and update the documents grid with a status chip (“Ready”, “Needs review”).
   - Provide a single CTA (“Open Google Doc”) so the path feels one-step.
